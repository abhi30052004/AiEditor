...
export async function POST(request) {
  try {
    ...
    if (!payload?.instruction || !String(payload.instruction).trim()) {
      return Response.json({ error: 'Rewrite instruction is required.' }, { status: 400 })
    }

    if (!payload?.currentContent || !String(payload.currentContent).trim()) {
      return Response.json({ error: 'There is no content in the editor to rewrite.' }, { status: 400 })
    }
    ...
    if (!content) {
      return Response.json({ error: 'The model returned an empty rewrite.' }, { status: 502 })
    }
    ...
    return Response.json(
      { error: 'Unable to rewrite content right now. Check your API key and try again.' },
      { status: 500 }
    )
  }
}
